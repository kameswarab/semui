...
@Component({
    selector: 'testOutput',
    templateUrl: './testOutput.component.html',
    styleUrls: []
})
export class TestOutputComponent implements OnInit {
    ...
    publishModel() {
        ...
        obj['modelId'] = this.modelId;
        obj['status'] = '35';
        this.modelsService.updateModelStatus(obj).subscribe(
            response => {
                this.showSuccessValidations(true, 'Your Model has been published and now ready to use the model.');
            },
            error => {
                this.showErrorValidations(true, error);
                console.log(error);
